 ...
 * @author Mr.Mic
 */
public class viweParentProfile extends HttpServlet {
    String userid, parameter,RequestType;
    ResultSet result;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet viweParentProfile</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet viweParentProfile at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        } finally {
            out.close();
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        userid = (String) request.getAttribute("userid");
            ...
            parameter = "('" + userid + "')";
            result = pc.callProc("selectGardian", parameter);
            try {
                if (result.next()) {
            ...
            RequestDispatcher rd = request.getRequestDispatcher("/updateParentData.jsp");
            rd.forward(request, response);
            }else{
            RequestDispatcher rd = request.getRequestDispatcher("/viweParenttProfile.jsp");
            rd.forward(request, response);
            }

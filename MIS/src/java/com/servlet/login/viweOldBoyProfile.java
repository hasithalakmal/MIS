 ...
 * @author Mr.Mic
 */
public class viweOldBoyProfile extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet viweOldBoyProfile</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet viweOldBoyProfile at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        } finally {
            out.close();
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       HttpSession session = request.getSession();
        ...
        parameter = "('" + userid + "')";
        result = pc.callProc("selectOldBoy", parameter);
        try {
            if (result.next()) {
        ...
        if ("rci".equals(user2)) {
            if ("update".equals(RequestType)) {
                RequestDispatcher rd = request.getRequestDispatcher("/updateOldBoyData.jsp");
                rd.forward(request, response);
            } else {
                RequestDispatcher rd = request.getRequestDispatcher("/viweOldBoyProfile.jsp");
                rd.forward(request, response);
            }
        } else if ("opi".equals(user2)) {

            if ("update".equals(RequestType)) {
                RequestDispatcher rd = request.getRequestDispatcher("/updateOldBoyData_OPI.jsp");
                rd.forward(request, response);
            } else {
                RequestDispatcher rd = request.getRequestDispatcher("/viweOldBoyProfile_OPI.jsp");
                rd.forward(request, response);
            }
        }
            }else{
